...
namespace Project.CoreDomain.Services.View
{
    public class ViewService : IViewService
    {
        ...
        public async UniTask<IDisposableView<T>> CreateAsync<T>(string assetId) where T : MonoBehaviour
        {
            var gameObjectKeeper = await _contentService.LoadAsync<GameObject>(assetId);
            
            var component = gameObjectKeeper.Value;
            var view = _diService.ContainerByScreenId[_screensService.Current].InstantiatePrefab(component);
